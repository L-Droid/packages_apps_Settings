...
package com.android.settings.ldroid.lockscreen;
...
public class LockscreenStyle extends SettingsPreferenceFragment
    ...
    private void resizeLDroidLock() {
        Bitmap ldroidLock = BitmapFactory.decodeResource(getResources(), R.drawable.ldroid_lock);
        if (ldroidLock != null) {
            String path = null;
            int px = requestImageSize();
            ldroidLock = Bitmap.createScaledBitmap(ldroidLock, px, px, true);
            try {
                mLockImage.createNewFile();
                mLockImage.setWritable(true, false);
                File image = new File(getActivity().getFilesDir() + File.separator
                ...
                ldroidLock.compress(Bitmap.CompressFormat.PNG, 100, outPut);
                image.setReadable(true, false);
                outPut.flush();
                outPut.close();
            } catch (Exception e) {
